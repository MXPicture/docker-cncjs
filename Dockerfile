...
# insert pendant
RUN git clone https://github.com/MXPicture/cncjs-pendant-tinyweb "$BUILD_DIR/cncjs_tinyweb"
RUN git clone https://github.com/MXPicture/cncjs-shopfloor-tablet "$BUILD_DIR/cncjs_shopfloor"
...
COPY --from=build /tmp/build/cncjs_tinyweb /opt/cncjs_tinyweb
COPY --from=build /tmp/build/cncjs_shopfloor /opt/cncjs_shopfloor

WORKDIR /opt/cncjs
EXPOSE 80
# CMD /opt/cncjs/entrypoint -H 0.0.0.0 -p 80 -c /config/cncjs.json -m /pendant:/opt/cncjs_tinyweb/src
